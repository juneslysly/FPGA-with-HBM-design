...
Novo-G, an influential supercomputing project in the field of Reconfigurable Computing, is being developed by a research team at the University of Florida. It is considered one of the pioneers in the field of scalable, reconfigurable supercomputing and aims to break through energy efficiency and performance bottlenecks by combining traditional CPUs with programmable hardware such as FPGAs.
% add comment for the Novo-G
...
parameters are read from the memory, such as A, B and other parameters in Figure\ref{3order}. %??? mark
...
The inputs to the third-order interpolating range limted force calculation unit 
...
'of the range limted force calculation unit block diagram.
...
As illustrated in Figure\ref{3DF}, direct DSP channel implementations achieve 32-bit 
...
As illustrated in Figures 5-15 and 5-16, the test system employed the 
...
In MD hardware design, the finite-range interaction force module effectively showcases the hardware's parallelism. This design employs classical acceleration algorithms like the proximity list lookup method and rationally designs the module by leveraging the performance of each expansion module of the FPGA device.
During the calculation of particle-pair interaction forces, a vast amount of intermediate computational data is generated. Such data requires proper splicing and caching, and relying solely on chip memory is insufficient.

An all-hardware module is designed to accelerate MD calculations, aiming to boost the calculation speed. The pipeline technique optimizes task allocation within the parallel system, and the Single Instruction Multiple Data Stream (SIMD) approach significantly enhances computational efficiency. Additionally, a dedicated software framework is established to accelerate circuit data processing.
To fully exploit the parallel potential, we compare existing high-speed interfaces and memory devices and analyze current high-speed architectural designs. New-generation high-bandwidth memories, such as HBM2, are used to assess the impact of existing memory structures on computational performance in MD design. Extensive work has been done with the DDR4 high-speed interface, and a polling algorithm is applied to improve access efficiency.

The new-generation high-speed, high-bandwidth multi-parallel interface HBM memory enables simultaneous read and write access to external mass storage during multi-path data parallel computation. The optimized design of the underlying RTL circuits fundamentally improves data read and write register efficiency, doubling the external storage read and write efficiency. Thus, the rational use of external mass memory devices is fully considered in the design.
...
At the hardware design level, the RTL language is used to complete the development of core modules. A parallel pipeline is implemented for the calculation of van der Waals forces. Through the Verlet neighbor list algorithm and hierarchical interpolation strategies (Newton/Hamilton interpolation), the calculation efficiency is increased by more than three times. In the limited-range force module, a multi-level bin division mechanism (16 bins in each of the first 7 segments + 128 bins in the last segment) is innovatively introduced, and a second-order polynomial fitting model is constructed to control the relative error to be less than 0.1\%. For the optimization of Coulomb forces, the PME algorithm is selected and a hierarchical state machine scheduling architecture is designed, which reduces the consumption of logic resources by 32\% while ensuring accuracy. Experimental data shows that FPGA demonstrates significant parallel advantages in non-bonded force calculations, and the characteristic that its DSP resource utilization rate is only 3\% reserves space for the deployment of larger-scale computing units.
