...
\noindent
Molecular dynamics (MD) simulations have shown critical importance in material and drug discovery due to its ability to provide detailed mechanistic insights into various material functions and biophysical interactions~\cite{fep_runtong,fep_york,md_in_dd_jmc2016,md_in_material1}. 
...
Additionally, the MD-GRAPE series, which dates back to the late 1980s, uses MCUs, Complex Programmable Logic Devices (CPLDs), FPGAs, and other acceleration circuits to build a multi-node design~\cite{MD-GRAPE-3,MDGRAPE-4A}.
...
\begin{center}
	\begin{equation}
		E_{Coulomb}=1/4\varepsilon \pi \cdot 1/2 \sum_{n}^{} \sum_{i=1}^{N} \sum_{j=1}^{N} \frac{q_{i}q_{j} }{\left|r_{ij}+nL\right|} 
	\end{equation}
\end{center}
